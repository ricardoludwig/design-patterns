...
class SingletonHolderTest {
    ...
    fun `test singleton returns same instance`() {
        val instance1 = SingletonHolder.getInstance("test")
        val instance2 = SingletonHolder.getInstance("different value")
    ...
    fun `test singleton thread safety`() {
        ...
        val instances = mutableListOf<SingletonHolder?>()

        // Create multiple threads that try to get the instance simultaneously
        repeat(threadCount) {
            executor.submit {
                val instance = SingletonHolder.getInstance("test$it")
    ...
    fun `test singleton remains same after multiple calls`() {
        val firstInstance = SingletonHolder.getInstance("initial")

        // Try to create new instances multiple times
        repeat(5) {
            val nextInstance = SingletonHolder.getInstance("attempt$it")
